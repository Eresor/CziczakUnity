...
public class BezierSplineInspector : Editor
{
    ...
    {
        Vector3 pointWorldPosition = handleTransform.TransformPoint(spline.GetControlPoint(index));
        Handles.color = modeColors[(int)spline.GetControlPointMode(index)];
        float size = HandleUtility.GetHandleSize(pointWorldPosition);
        if (Handles.Button(pointWorldPosition, handleRotation, size * 0.05f, size * 0.07f, Handles.DotHandleCap))
        {
            selectedIndex = index;
            Repaint();
        }
